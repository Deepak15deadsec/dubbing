...
const Sidebar = () => {
  ...
  const sideMenu = () => {
    return (
      <Menu className="text-black" renderExpandIcon={({ open }) => <span>{open ? '-' : '+'}</span>}>
        <SubMenu className={`${active === "campaigns" ? "text-[#01A4EF]" : "text-black"}`} defaultOpen label="Campaigns "
        ...
        <SubMenu className={`${active === "red" ? "text-[#01A4EF]" : "text-black"} `} label="Profile & Settings" icon={<img src={profileSetting} />}>
  ...
  return (
    <SideBar backgroundColor="#fff" style={{ border: "none" }} breakPoint="sm" transitionDuration={1000} className="w-full min-h-screen">
      <div className={`w-full flex flex-col items-center `}>

        <div className="mt-[3rem]"> <img
